...
#' data("MVC.ST")
#' data("MVC.ST.coor")
#' database <- data_process(MVC.reference, MVC.reference.cell.label, MVC.ST, MVC.ST.coor)
#'
#' @export
...
data_process <- function(sc_exp, sc_label, spot_exp, spot_loc,
  ...
  database <- list(sc_exp = sc_exp, sc_label = sc_label, spot_exp = st_exp, spot_loc = spot_loc)
  return(database)
}
...
DWLS_run = function(database, parallel = TRUE,  is_select_DEGs = TRUE, python_env){
  ...
  DWLS <- list(DampenedWLS = DampenedWLS, SVR = SVR)
...
Stereoscope_run <- function(database, python_env = NULL, use_gpu = FALSE,
                            sc_training_plot = FALSE, sc_training_save_trained_model = FALSE, sc_max_epochs = 400, sc_lr = 0.01,
                            st_training_plot = FALSE, st_training_save_trained_model = FALSE, st_max_epochs = 400, st_lr = 0.01){
  ...
  scvi$external$stereoscope$SpatialStereoscope$train(st_model, max_epochs = as.integer(5000), check_val_every_n_epoch = as.integer(1),
...
#'
#' @return a list contains all the results inferred by individual deconvolution methods. The elements of list is a matrix, spots * cell-type.
#'
#' @export
EnDecon_individual_methods <- function(sc_exp, sc_label, spot_exp, spot_loc,
                                       ...
                                       SCDC = TRUE, RCTD = TRUE, MuSiC = TRUE, DeconRNASeq = TRUE, DWLS = TRUE, SPOTlight = TRUE,
                                       SpatialDWLS = TRUE, Stereoscope = TRUE, cell2location = TRUE, CARD = TRUE,
                                       ...
                                       Stereoscope.sc_max_epochs = 400, Stereoscope.sc_lr = 0.01,
                                       Stereoscope.st_training_plot = FALSE, Stereoscope.st_training_save_trained_model = FALSE,
                                       Stereoscope.st_max_epochs = 400, Stereoscope.st_lr = 0.01,
  ...
  Methods <- c("CARD", "cell2location", "DeconRNASeq", "DWLS", "MuSiC", "RCTD", "SCDC", "SpatialDWLS","SPOTlight","Stereoscope")

  Methods.idx <- c(CARD, cell2location, DeconRNASeq, DWLS, MuSiC, RCTD, SCDC, SpatialDWLS,SPOTlight,Stereoscope)
...
solve_ensemble <- function(Results.Deconv, lambda = NULL, prob.quantile = 0.5,
  ...
  while (k <= niter) {
    if(k == 1){
      loss_all_temp <- 0
      temp2 <-  sapply(Results.Deconv, L1_norm, Y = V)
    ...
    V <- apply(temp4, 1:2, median_weighted, w = w)

    loss_main <- sum(sapply(Results.Deconv, L1_norm, Y = V) * w)
  ...
  colnames(V) <- colnames(Results.Deconv[[1]])
  V_norm <- sweep(V, 1, rowSums(V), "/")
  return(list(V_norm = V_norm, w = w))
}
